...
For pilot demos, the Actions layer also includes deterministic fallback catalogs. The regional fallback covers common geographies such as New York City, Washington DC, San Francisco Bay Area, Los Angeles, Chicago, Boston, Seattle, Atlanta, Philadelphia, Miami, and Dallas. The cause fallback covers global health and digital health patterns such as telemedicine, maternal health, primary care, South Asia, India, Nepal, and Kyrgyzstan. Seed candidates keep demos from depending entirely on live search ranking. They are labeled `regional_fallback_seed` or `cause_fallback_seed`, scored as low confidence, and require verification of current 990 data, guidelines, recent peer grants, invitation status, grant-size fit, and relationship path before outreach.
